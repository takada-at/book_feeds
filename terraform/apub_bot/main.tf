...
resource "google_logging_project_sink" "log-sink" {
  ...
  filter      = "resource.type = \"cloud_run_revision\" AND resource.labels.service_name = \"${data.google_cloud_run_service.default.name}\" AND resource.labels.location=\"${var.region}\" AND jsonPayload.log_type = \"like\""
